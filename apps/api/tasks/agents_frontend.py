...
""",
    ...
    # Prefer SSH remote if available; deploy key mounted in container
    try:
        out = subprocess.check_output(["git", "config", "--get", "remote.origin.url"], cwd=str(repo_root)).decode().strip()
        if out.startswith("https://"):
            ix = out.find("github.com/")
            slug = out[ix + len("github.com/") :] if ix != -1 else None
            if slug and slug.endswith(".git"):
                slug = slug[:-4]
            if slug:
                ssh_url = f"git@github.com:{slug}.git"
